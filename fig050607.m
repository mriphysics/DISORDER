function fig050607(id,quick,pr)

%FIG050607  Generates Figs. 5, 6 and 7 of the manuscript "Motion corrected
...
%and JV Hajnal
%   FIG050607({ID},{ACCEL},{PR})
%   * {ID} indicates the figure to generate, either '05', '06' or '07', if
...
%   plots and showing reconstruction results for theta=5/S=16 (Fig. 5) and 
%   S=25/R=1.6x1.6 (Fig. 7) instead of the more challenging theta=10/S=64 
%   (Fig. 5) and S=16/R=2x2 (Fig. 7). If 0 it runs the tests in the 
%   manuscript
...
if nargin<1 || isempty(id);id={'05','06','07'};elseif ~iscell(id);id={id};end
...
for ii=1:length(id)
    clearvars -except id quick pr pathData ii

    %EXPERIMENT ID
    figN=id{ii};%From '05' to '07'
    if ~(strcmp(figN,'05') || strcmp(figN,'06') || strcmp(figN,'07'));fprintf('Figure %s not contemplated\n',id{ii});return;end
    ...
    if ismember(pr,0:1)
        ...
        load(sprintf('%s/xGT',pathData),'xGT','S');
        if gpu;xGT=gpuArray(xGT);S=gpuArray(S);end

        %EXPERIMENT PARAMETERS
        [theta,NS,recTyp]=generateParametersExp(figN,quick);
        ...
        if pr
            load(sprintf('%s/Fig%s',pathData,figN),'TGT','TFov','no','A','B');
            if gpu
        ...
        %ITERATIONS
        if strcmp(figN,'05');nExtern=20000;
        elseif strcmp(figN,'06');nExtern=200000;
        elseif strcmp(figN,'07');nExtern=200000;
        ...
        %WRITE DATA
        save(sprintf('%s/Fig%s',pathData,figN),'xNOTR','xEst','xGT','xGTNO','xGTNOTR','TEst','TGT','TFov','rEst','A','B','no','errFGTNO','errXGTNO','errFGTNOTR','errXGTNOTR','effIt','errX','errF','theta','NS','recTyp','nExtern','snrdB');
    else
        load(sprintf('%s/Fig%s',pathData,figN),'xNOTR','xEst','xGT','xGTNO','xGTNOTR','TEst','TGT','TFov','rEst','A','B','no','errFGTNO','errXGTNO','errFGTNOTR','errXGTNOTR','effIt','errX','errF','theta','NS','recTyp','nExtern','snrdB');
    end
        
    if strcmp(figN,'07');dfv=1;else dfv=0;end
    ...
    for e=1:E
        if strcmp(figN,'05');lab{e}=cell2mat(recTyp(e,1));
        elseif strcmp(figN,'06');lab{e}=strcat(cell2mat(recTyp(e,1)),sprintf(' ($L=%s$)',cell2mat(recTyp(e,2)))); 
        elseif strcmp(figN,'07');lab{e}=strcat(cell2mat(recTyp(e,1)),sprintf(' ($L=%s$)',cell2mat(recTyp(e,2))));
        end
    end
    ...
    for s=1:S
        for v=1:V    
            ...
            if ~strcmp(figN,'07');semilogy(1:limsaxis(2),(errFGTNO{s}/rangYRef(s))*ones(1,limsaxis(2)),'Color',co(end,:),'LineWidth',1,'LineStyle','--');end
            axis(limsaxis)
            ...
            if strcmp(figN,'05');text(nExtern/10,rangY(s,1)*1.2,sprintf('$\\theta=$%d$^{\\circ}$ / $S=$%d',theta(1,v),NS(1,s)),'interpreter','latex','Fontsize',FontSizeB);
            elseif strcmp(figN,'06');text(nExtern/10,rangY(s,1)*1.2,sprintf('$\\theta=$%d$^{\\circ}$ / $S=$%d',theta(1,v),NS(1,s)),'interpreter','latex','Fontsize',FontSizeB);
            elseif strcmp(figN,'07');text(nExtern/10,rangY(s,1)*1.2,sprintf('$\\theta=$%d$^{\\circ}$ / $S=$%d / $R=$%.1f$\\times$%.1f',theta(1,v+1),NS(1,s),NS(2,s),NS(2,s)),'interpreter','latex','Fontsize',FontSizeB);
            end
    ...
    if strcmp(figN,'05')
        ...
        %5deb, 16 shots (quick)
        xFig{1}=xGTNO{2}{1};
        xFig{2}=xNOTR{3-quick}{3-quick}{1};
        xFig{3}=xEst{3-quick}{3-quick}{1};
        xFig{4}=xNOTR{3-quick}{3-quick}{4};
        xFig{5}=xEst{3-quick}{3-quick}{4};
        pause(1);figure;pause(1)
    ...
    if strcmp(figN,'07')             
        ...
        xFig{3}=xEst{1}{3}{3};
        xFig{4}=xGTNOTR{3-quick}{1}{3}{NL};
        xFig{5}=xGTNOTR{3-quick}{3}{3}{NL};
        xFig{6}=xEst{3-quick}{3}{3};
        pause(1);figure;pause(1)
        imshow(1000*abs(cat(2,xFig{:})),[0 25],'Border','tight')
        colorbar('FontSize',FontSizeD);
        set(gcf, 'Position', get(0,'Screensize'),'Color',[1 1 1])
