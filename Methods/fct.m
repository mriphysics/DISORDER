function x=fct(x,n,dim)

%FCT   Performs a Discrete Cosine Transform along a given dimension. It is
%based on Andriy Myronenko's implementation in 
...
%COSINE TRANSFORM
ind=[1:2:n flip(2:2:n)];
x=dynInd(x,ind,dim);%Reorder
isrealx=isreal(x);
if ~isrealx;x=cat(length(M)+1,real(x),imag(x));end    
x=fft(x,[],dim);%ifft
x=real(bsxfun(@times,ww,x));%Weight
